...
inline void write_limit_output_file(const std::string &fname, const std::vector<limit_result> &results)
{
	...
	for (int i = 0; i < results.size(); i++) {
		...
		h_p1sigma->SetBinContent(i + 1, results[i].cl_p1sigma * results[i].signal.signalEvents.A / results[i].signal.efficiency[A]);
		h_p2sigma->SetBinContent(i + 1, results[i].cl_p2sigma * results[i].signal.signalEvents.A / results[i].signal.efficiency[A]);
		h_n1sigma->SetBinContent(i + 1, results[i].cl_n1sigma * results[i].signal.signalEvents.A / results[i].signal.efficiency[A]);
		h_n2sigma->SetBinContent(i + 1, results[i].cl_n2sigma * results[i].signal.signalEvents.A / results[i].signal.efficiency[A]);
	}
