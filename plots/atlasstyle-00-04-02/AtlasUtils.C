...
void myBoxText(Double_t x, Double_t y,Double_t boxsize,Int_t mcolor,const char *text) 
{
  ...
  TLatex l; l.SetTextAlign(12); //l.SetTextSize(tsize); 
  l.SetNDC();
  l.DrawLatex(x+1.2*boxsize,y,text);

  Double_t y1=y-0.25*tsize;
  Double_t y2=y+0.25*tsize;
  Double_t x2=x+boxsize;//-0.3*tsize;
  Double_t x1=x;//2-boxsize;
...
void myBoxTextDash(Double_t x, Double_t y,Double_t boxsize,Int_t mcolor,Int_t lcolor,const char *text, double lstyle){

  Double_t tsize=gStyle->GetTextSize();
